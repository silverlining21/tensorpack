...
__all__ = ['IDCardMeta', 'IDCard12']

CAFFE_IDCard12_URL = "http://dl.caffe.berkeleyvision.org/caffe_ilsvrc12.tar.gz"

class IDCardMeta(object):
    """
    Some metadata for IDCard dataset.
    """
    def __init__(self, dir=None):
        if dir is None:
            dir = get_dataset_path('/home/nrp/data/idcard')
        self.dir = dir
        mkdir_p(self.dir)
    ...
    def _download_caffe_meta(self):
        fpath = download(CAFFE_IDCard12_URL, self.dir)
    ...
    def get_per_pixel_mean(self, size=None):
        ...
        mean_file = os.path.join(self.dir, 'mean.binaryproto')
        with open(mean_file, 'rb') as f:
            obj.ParseFromString(f.read())
        arr = np.array(obj.data).reshape((1, 32, 32)).astype('float32')
        # arr = np.transpose(arr, [1,2,0])
        if size is not None:
            arr = cv2.resize(arr, size[::-1])
        return arr

class IDCard12(RNGDataFlow):
    def __init__(self, dir, name, meta_dir=None, shuffle=True,
        ...
        :param dir: A directory containing a subdir named `name`, where the
            original IDCard12_`name`.tar gets decompressed.
        :param name: 'train' or 'val' or 'test'
        ...
        After decompress IDCard12_img_train.tar, you can use the following
        command to build the above structure for `train/`:

        .. code-block:: none

            tar xvf IDCard12_img_train.tar -C train && cd train
        ...
        if include_bb:
            ...
            self.bblist = IDCard12.get_training_bbox(bbdir, self.imglist)
    ...
    def get_data(self):
        ...
        for k in idxs:
            ...
            im = cv2.imread(fname.strip(), cv2.IMREAD_GRAYSCALE)
...
if __name__ == '__main__':
    ...
    ds = IDCard12('/home/wyx/data/fake_ilsvrc/', 'train', include_bb=True,
            shuffle=False)
    ds.reset_state()
